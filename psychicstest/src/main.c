...
void scrollScreen(i16 scroll) {
  ...
  // When scrolling to the left, erase the character that appears on the left,
  // just
  ...
  if (AABB_BoxCollision(&character->body.box, platform)) {
    switch (checkCollisionSide(platform, &character->body.box)) {
      case COLLISION_SIDE_BOTTOM:
        ...
        character->body.box.min.y =
            platform->min.y - character->body.box.size.y;
        break;
      case COLLISION_SIDE_TOP:
        character->body.velocity.y = 0;
        character->body.box.min.y = platform->max.y + 1;
        character->body.box.max.y =
            character->body.box.min.y + character->body.box.size.y;
        break;
      case COLLISION_SIDE_RIGHT:
        character->body.box.max.x = platform->min.x;
        character->body.box.min.x =
            platform->min.x - character->body.box.size.x;
        character->body.velocity.x = 0;
        break;
      case COLLISION_SIDE_LEFT:
        character->body.box.min.x = platform->max.x;
        character->body.box.max.x =
            platform->max.x + character->body.box.size.x;
        character->body.velocity.x = 0;
        break;
    }
  }
